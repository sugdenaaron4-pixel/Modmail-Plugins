...
class Jet2Support(commands.Cog):
    ...
    async def send_queue_update(self, thread, department_id):
        ...
        embed = discord.Embed(
            title="📋 Queue Update",
            description=(
                f"You are currently **{position}{suffix}** in the "
                f"**{DEPARTMENTS[department_id]['name']}** queue."
            ),
            color=self.color
        )
    ...
    async def on_thread_claim(self, thread, user):
        ...
        embed = discord.Embed(
            ...
            description=(
                "A member of the **Jet2Support** team "
